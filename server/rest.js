...
JsonRoutes.add("get", "/fhir/Immunization", function (req, res, next) { process.env.DEBUG && console.log('GET /fhir/Immunization', req.query);
  ...
  if (accessToken || process.env.NOAUTH) {
    ...
    process.env.DEBUG && console.log('Immunizations.find(id)', Immunizations.find(databaseQuery).fetch()); // because we're using BaseModel and a _transform() function
    // Immunizations returns an object instead of a pure JSON document // it stores a shadow reference of the original doc, which we're removing here
    var immunizationData = Immunizations.find(databaseQuery).fetch();
    immunizationData.forEach(function(patient){
